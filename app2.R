...
PPGISr <- function(base_map, information_layers, mapping_categories, mapping_colors, message){ 
  if (!exists("base_map")) {
    VECTOR_FILE <<- st_read("C:/Data/PIVOT/PIVOT/Duluth_bndry_hex.gpkg") %>%
  ...
  if (!exists("information_layers")) {
    ADDITIONAL_LAYER <<- st_read("C:/Data/PIVOT/PIVOT/Duluth_Vulnerability.shp")
  } else {
    ADDITIONAL_LAYER <- st_read(information_layers)
  }
  ...
  if (!exists("mapping_colors")) {
    COLOR_PAL <- c("#880015", "#22b14c", "#00a2e8")
  } else {
    COLOR_PAL <- mapping_colors
  }
  if (!exists("note")) {
    NOTE <- "Use PPGISr to prioritize locations on a map"
  } else {
    NOTE <- message
  }
  #####################################################################################################
